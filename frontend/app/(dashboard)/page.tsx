...
import { Wand2, Leaf, Film, Type, PenTool, Scissors, ScanText, Newspaper, Gem, Package } from "lucide-react";
...
// Kept brisk on purpose: with eleven cards, a slower stagger means the last one
    ...
    <div className="flex-1 overflow-y-auto px-8 py-8">
      <div className="max-w-6xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
          <h1 className="text-cream text-2xl font-bold">Welcome back{firstName ? `, ${firstName}` : ""} 👋</h1>
          <p className="text-muted text-sm mt-1">Pick a tool below to start transforming your jewellery visuals.</p>
        </motion.div>
